...
export class CarService {
  ...
  async create(createCarDto: CreateCarDto) {
    try {
      ...
      // const loc_exceptions = createCarDto.location_exceptions;
      ...
      if (createCarDto.is_car_details_enabled) {
        const { car_hourly_details, car_per_mile_details } = createCarDto ?? {};
        let carPriceTypeId = 0;
        if (
          Object.values(car_hourly_details).every((val) => +val !== 0) &&
          ...
          carPriceTypeId = Object.values(car_hourly_details).every(
            (val) => +val !== 0,
          )
            ? 1
            : 2;
          const carDetails = Object.values(car_hourly_details).every(
            (val) => +val !== 0,
          )
            ? car_hourly_details
      ...
      // Create location exceptions
      // if (createCarDto.is_loc_enabled && loc_exceptions.length > 0) {
      //   const res = loc_exceptions.map((loc_exception) => {
      //     return {
      //       pick_up_postal_code: loc_exception.pick_up_postal_code,
      //       drop_off_postal_code: loc_exception.drop_off_postal_code,
      //       price: loc_exception.price,
      //     };
      //   });
      //   await this.prismaService.location_exceptions.createMany({
      //     data: res,
      //   });
      // }
      // Create car features
  ...
  async findOne(id: number) {
    try {
      ...
      // Get all slabs and location exceptions for the car
  ...
  async update(id: number, updateCarDto: UpdateCarDto) {
    try {
      ...
      const updateCar = await this.prismaService.cars.update({
        ...
        data: {
          body_type_id:
            Number(updateCarDto.body_type_id) || singleCar.body_type_id,
          transmission_id:
            Number(updateCarDto.transmission_id) || singleCar.transmission_id,
          manufacturer_id:
            Number(updateCarDto.manufacturer_id) || singleCar.manufacturer_id,
          driver_id: Number(updateCarDto.driver_id) || singleCar.driver_id,
          fuel_id: Number(updateCarDto.fuel_id) || singleCar.fuel_id,
          vehicle_type_id:
            Number(updateCarDto.vehicle_type_id) || singleCar.vehicle_type_id,
          ...
          passenger_seats:
            Number(updateCarDto.passenger_seats) || singleCar.passenger_seats,
          maximum_luggage:
            Number(updateCarDto.maximum_luggage) || singleCar.maximum_luggage,
          image: updateCarDto.image || singleCar.image,
          total_doors:
            Number(updateCarDto.total_doors) || singleCar.total_doors,
          child_seat: Number(updateCarDto.child_seat) || singleCar.child_seat,
          status: updateCarDto.status,
          per_mile_rate:
            Number(updateCarDto.per_mile_rate) || singleCar.per_mile_rate,
          hourly_rate:
            Number(updateCarDto.hourly_rate) || singleCar.hourly_rate,
          minimum_fare:
            Number(updateCarDto.minimum_fare) || singleCar.minimum_fare,
          minimum_hours:
            Number(updateCarDto.minimum_hours) || singleCar.minimum_hours,
          fixed_price:
            Number(updateCarDto.fixed_price) || singleCar.fixed_price,
  ...
  async addSlab(car_id: number, dto: CreateSlabDto) {
    try {
  ...
  async updateSlab(id: number, dto: UpdateSlabDto) {
    try {
  ...
  // async addLocationException(car_id: number, dto: CreateLocExceptionDto) {
  //   try {
  //     // Check if the car exists
  //     const carExists = await this.prismaService.cars.findUnique({
  //       where: {
  ...
  //       };
  //     }
  //     // Check if the location exceptions already exist
  //     const locationExceptionExists =
  ...
  //     if (locationExceptionExists) {
  //       return {
  //         status: 'failed',
  //         message: 'Location exception already exists!',
  //         data: {},
  //       };
  //     }
  ...
  // async updateLocationException(id: number, dto: UpdateLocExceptionDto) {
  //   try {
  ...
  // async deleteLocationException(id: number) {
  //   try {
